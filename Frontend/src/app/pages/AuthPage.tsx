...
export function AuthPage({ mode: initialMode }: AuthPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.trim()) {
      ...
      newFieldErrors.email = t('auth.emailInvalid');
    }

    if (!password) {
      newFieldErrors.password = t('auth.passwordRequired');
    } else if (password.length < 6) {
      newFieldErrors.password = t('auth.passwordLength');
    }
    ...
    try {
      ...
      if (msg.includes('not approved')) {
        setError(t('auth.pendingApprovalNotice'));
      } else if (msg.toLowerCase().includes('email')) {
        setFieldErrors({ email: msg.replace(/"/g, '') });
      } else if (msg.toLowerCase().includes('password')) {
        setFieldErrors({ password: msg.replace(/"/g, '') });
      } else {
        setError(msg || (initialMode === 'login' ? t('auth.invalidLogin') : t('auth.failedRegister')));
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="w-full bg-[#2D6DB5] flex-shrink-0 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-white text-xl font-bold tracking-wide">{t('signin.title')}</h1>
            ...
            <div className="p-6 sm:p-8">
              <div className="text-center mb-6">
                <h2 className="text-xl font-bold text-gray-900">
                  {initialMode === 'login' ? t('auth.welcomeBack') : t('auth.createAccount')}
                </h2>
